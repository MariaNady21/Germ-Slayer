...
public class LevelLayoutEditor : Editor
{
    ...
    void OnSceneGUI()
    {
        if (!m_EditingLayout) return;

        foreach (var room in m_LevelLayout.rooms)
        {
            ...
            // مكان عرض النص (يمكن تغييره بحسب الحاجة)
            Vector3 labelPos = bounds.center + Vector3.up * (bounds.size.y / 2 + 0.2f);

            // نص المقاسات
            string sizeText = $"W: {size.x:F2}\nH: {size.y:F2}\nD: {size.z:F2}";

            // عرض النص في المشهد
            Handles.color = Color.white;
            Handles.Label(labelPos, sizeText);
        }
    }
